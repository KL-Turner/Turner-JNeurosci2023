...
for qq = 1:size(procDataFileIDs,1)
    ...
    if isfield(ProcData.data.Pupil,'accessoryPatch') == false
        ...
        [linkedBlinkIndex] = LinkBinaryEvents_Turner2022(gt(blinkNaNs,0),[30,0]); % link greater than 1 second
        ...
        [linkedDiffIndex] = LinkBinaryEvents_Turner2022(gt(diffIndex,0),[30*2,0]);
        % identify edges for interpolation
        edgeFoundB = false;
        xx = 1;
        startEdgeB = [];
