function [] = RunPupilTracker_JNeurosci2023(procDataFileIDs)
...
for bb = 1:length(firstFileOfDay)
    ...
    load(firstFile)
    [animalID,fileDate,fileID] = GetFileInfo_JNeurosci2023(firstFile);
...
for cc = 1:size(procDataFileIDs,1)
    ...
    if isfield(ProcData.data,'Pupil') == false
        ...
        [animalID,fileDate,fileID] = GetFileInfo_JNeurosci2023(procDataFileID);
        strDay = ConvertDate_JNeurosci2023(fileDate);
        pupilCamFileID = [fileID '_PupilCam.bin'];
        idx = 0;
        ...
        [linkedBlinkIndex] = LinkBinaryEvents_JNeurosci2023(gt(blinkNaNs,0),[samplingRate,0]); % link greater than 1 second
        ...
        [linkedDiffIndex] = LinkBinaryEvents_JNeurosci2023(gt(diffIndex,0),[samplingRate*2,0]);
        % identify edges for interpolation
        edgeFoundB = false;
        xx = 1;
        startEdgeB = [];
