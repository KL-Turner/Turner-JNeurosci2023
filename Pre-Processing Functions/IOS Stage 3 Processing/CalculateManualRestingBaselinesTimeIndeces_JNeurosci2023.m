function [RestingBaselines] = CalculateManualRestingBaselinesTimeIndeces_JNeurosci2023(imagingType,hemoType)
...
for a = 1:size(procDataFileIDs,1)
    ...
    if isfield(ProcData,'manualBaselineInfo') == false
        ...
        while b == false
            ...
            [singleTrialFig] = GenerateSingleFigures_JNeurosci2023(procDataFileID,RestingBaselines,baselineType,saveFigs,imagingType,hemoType);
...
for e = 1:length(dataTypes)
    ...
    if strcmp(dataType,'Pupil') == false
        ...
        for f = 1:length(subDataTypes)
            ...
            [restLogical] = FilterEvents_JNeurosci2023(RestData.(dataType).(subDataType),RestCriteria);
            [puffLogical] = FilterEvents_JNeurosci2023(RestData.(dataType).(subDataType),PuffCriteria);
            ...
            uniqueDays = GetUniqueDays_JNeurosci2023(RestData.(dataType).(subDataType).fileIDs);
            ...
            for p = 1:length(uniqueDays)
                ...
                for r = 1:length(finalEventFileIDs)
                    ...
                    uniqueDate{p,1} = ConvertDate_JNeurosci2023(uniqueDays{p,1});
