function [] = CategorizeData_JNeurosci2022(procDataFileID,stimulationType)
...
binWhiskers = LinkBinaryEvents_JNeurosci2022(gt(modBinWhiskers,0),[linkThresh breakThresh]*whiskerSamplingRate);
...
[ProcData.flags.whisk] = GetWhiskingdata_JNeurosci2022(ProcData,binWhiskers);
% Retrieve details on puffing events
[ProcData.flags.stim] = GetStimdata_JNeurosci2022(ProcData);
...
function [puffTimes] = GetPuffTimes_JNeurosci2022(ProcData)
try
...
function [Stim] = GetStimdata_JNeurosci2022(ProcData)
% Setup
whiskerSamplingRate = ProcData.notes.dsFs;
forceSensorSamplingRate = ProcData.notes.dsFs;
puffTimes = GetPuffTimes_JNeurosci2022(ProcData);
...
function [Whisk] = GetWhiskingdata_JNeurosci2022(ProcData,binWhiskerAngle)
% Setup
whiskerSamplingRate = ProcData.notes.dsFs;
forceSensorSamplingRate = ProcData.notes.dsFs;
% Get Puff Times
...
binWhiskerAngle = LinkBinaryEvents_JNeurosci2022(gt(modBinarizedWhiskers,0),[linkThresh breakThresh]*whiskerSamplingRate);
binForceSensor = LinkBinaryEvents_JNeurosci2022(modBinarizedForceSensor,[linkThresh breakThresh]*forceSensorSamplingRate);
