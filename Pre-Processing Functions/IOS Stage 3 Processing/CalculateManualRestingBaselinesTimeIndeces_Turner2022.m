function [RestingBaselines] = CalculateManualRestingBaselinesTimeIndeces_Turner2022(imagingType,hemoType)
...
for a = 1:size(procDataFileIDs,1)
    ...
    if isfield(ProcData,'manualBaselineInfo') == false
        ...
        while b == false
            ...
            [singleTrialFig] = GenerateSingleFigures_Turner2022(procDataFileID,RestingBaselines,baselineType,saveFigs,imagingType,hemoType);
...
for e = 1:length(dataTypes)
    ...
    if strcmp(dataType,'Pupil') == false
        ...
        for f = 1:length(subDataTypes)
            ...
            [restLogical] = FilterEvents_Turner2022(RestData.(dataType).(subDataType),RestCriteria);
            [puffLogical] = FilterEvents_Turner2022(RestData.(dataType).(subDataType),PuffCriteria);
            ...
            uniqueDays = GetUniqueDays_Turner2022(RestData.(dataType).(subDataType).fileIDs);
            ...
            for p = 1:length(uniqueDays)
                ...
                for r = 1:length(finalEventFileIDs)
                    ...
                    uniqueDate{p,1} = ConvertDate_Turner2022(uniqueDays{p,1});
