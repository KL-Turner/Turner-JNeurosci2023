function [] = CategorizeData_Turner2022(procDataFileID,stimulationType)
...
binWhiskers = LinkBinaryEvents_Turner2022(gt(modBinWhiskers,0),[linkThresh breakThresh]*whiskerSamplingRate);
...
[ProcData.flags.whisk] = GetWhiskingdata_Turner2022(ProcData,binWhiskers);
% retrieve details on puffing events
[ProcData.flags.stim] = GetStimdata_Turner2022(ProcData);
...
function [puffTimes] = GetPuffTimes_Turner2022(ProcData)
try
...
function [Stim] = GetStimdata_Turner2022(ProcData)
% setup
whiskerSamplingRate = ProcData.notes.dsFs;
forceSensorSamplingRate = ProcData.notes.dsFs;
puffTimes = GetPuffTimes_Turner2022(ProcData);
...
function [Whisk] = GetWhiskingdata_Turner2022(ProcData,binWhiskerAngle)
% setup
whiskerSamplingRate = ProcData.notes.dsFs;
forceSensorSamplingRate = ProcData.notes.dsFs;
% get Puff Times
...
binWhiskerAngle = LinkBinaryEvents_Turner2022(gt(modBinarizedWhiskers,0),[linkThresh breakThresh]*whiskerSamplingRate);
binForceSensor = LinkBinaryEvents_Turner2022(modBinarizedForceSensor,[linkThresh breakThresh]*forceSensorSamplingRate);
