function [SpecData] = NormalizeSpectrograms_JNeurosci2022(neuralDataTypes,RestingBaselines)
...
for aa = 1:size(specDataFileIDs,1)
    ...
    [~,fileDate,~] = GetFileInfo_JNeurosci2022(specDataFileIDs(aa,:));
...
for cc = 1:size(specDataFileIDs,1)
    ...
    [~,fileDate,~] = GetFileInfo_JNeurosci2022(specDataFileIDs(cc,:));
    strDay = ConvertDate_JNeurosci2022(fileDate);
...
for ee = 1:size(specDataFileIDs,1)
    ...
    [~,fileDate,~] = GetFileInfo_JNeurosci2022(specDataFileIDs(ee,:));
    strDay = ConvertDate_JNeurosci2022(fileDate);
