function [DataStruct] = NormBehavioralDataStruct_JNeurosci2022(DataStruct,RestingBaselines,baselineType)
                ...
                if isfield(DataStruct.(dataType).(hemDataType),'whisk')
                    ...
                    for bF = 1:length(behaviorFields)
                        ...
                        if ~isempty(DataStruct.(dataType).(hemDataType).(behField).data)
                            ...
                            [uniqueDays,~,~] = GetUniqueDays_JNeurosci2022(DataStruct.(dataType).(hemDataType).(behField).fileDates);
                            for uD = 1:length(uniqueDays)
                                date = uniqueDays{uD};
                                strDay = ConvertDate_JNeurosci2022(date);
                                [~,dayInds] = GetDayInds_JNeurosci2022(DataStruct.(dataType).(hemDataType).(behField).fileDates,date);
                    ...
                    [uniqueDays,~,~] = GetUniqueDays_JNeurosci2022(DataStruct.(dataType).(hemDataType).fileDates);
                    for uD = 1:length(uniqueDays)
                        date = uniqueDays{uD};
                        strDay = ConvertDate_JNeurosci2022(date);
                        [~,dayInds] = GetDayInds_JNeurosci2022(DataStruct.(dataType).(hemDataType).fileDates,date);
