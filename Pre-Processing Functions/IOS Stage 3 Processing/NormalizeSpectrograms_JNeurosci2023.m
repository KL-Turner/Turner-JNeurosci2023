function [SpecData] = NormalizeSpectrograms_JNeurosci2023(neuralDataTypes,RestingBaselines)
...
for aa = 1:size(specDataFileIDs,1)
    ...
    [~,fileDate,~] = GetFileInfo_JNeurosci2023(specDataFileIDs(aa,:));
...
for cc = 1:size(specDataFileIDs,1)
    ...
    [~,fileDate,~] = GetFileInfo_JNeurosci2023(specDataFileIDs(cc,:));
    strDay = ConvertDate_JNeurosci2023(fileDate);
...
for ee = 1:size(specDataFileIDs,1)
    ...
    [~,fileDate,~] = GetFileInfo_JNeurosci2023(specDataFileIDs(ee,:));
    strDay = ConvertDate_JNeurosci2023(fileDate);
