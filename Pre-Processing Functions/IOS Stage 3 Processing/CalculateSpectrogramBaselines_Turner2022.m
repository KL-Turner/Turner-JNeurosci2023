function [RestingBaselines] = CalculateSpectrogramBaselines_Turner2022(animal,neuralDataTypes,trialDuration_sec,RestingBaselines,baselineType)
...
for a = 1:length(neuralDataTypes)
    ...
    for d = 1:length(restFileList)
        ...
        strDay = ConvertDate_Turner2022(fileID(1:6));
    ...
    uniqueDays = GetUniqueDays_Turner2022(RestingBaselines.(baselineType).baselineFileInfo.fileIDs);
    for f = 1:length(uniqueDays)
        g = 1;
        for field = 1:length(fields)
            if strcmp(fields{field}(7:12),uniqueDays{f})
                stringDay = ConvertDate_Turner2022(uniqueDays{f});
