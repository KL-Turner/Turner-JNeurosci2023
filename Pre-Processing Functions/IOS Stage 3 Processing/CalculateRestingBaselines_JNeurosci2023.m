function [RestingBaselines] = CalculateRestingBaselines_JNeurosci2023(animal,targetMinutes,trialDuration_sec,RestData)
...
for a = 1:length(dataTypes)
    ...
    for b = 1:length(subDataTypes)
        ...
        [restLogical] = FilterEvents_JNeurosci2023(RestData.(dataType).(subDataType),RestCriteria); % output is a logical
        [puffLogical] = FilterEvents_JNeurosci2023(RestData.(dataType).(subDataType),puffCriteria); % output is a logical
        ...
        uniqueDays = GetUniqueDays_JNeurosci2023(RestData.(dataType).(subDataType).fileIDs); % find the unique days of imaging
        ...
        for e = 1:length(uniqueDays)
            ...
            for f = 1:length(finalFileIDs)
                ...
                date{e,1} = ConvertDate_JNeurosci2023(uniqueDays{e,1});
