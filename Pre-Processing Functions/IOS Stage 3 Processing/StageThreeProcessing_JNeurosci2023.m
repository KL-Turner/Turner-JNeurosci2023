...
[animalID,~,~] = GetFileInfo_JNeurosci2023(procDataFileIDs(1,:));
...
for a = 1:size(procDataFileIDs,1)
    ...
    CategorizeData_JNeurosci2023(procDataFileID,stimulationType)
end
% create RestData data structure
[RestData] = ExtractRestingData_JNeurosci2023(procDataFileIDs,dataTypes,imagingType);
% analyze the spectrogram for each session.
CreateTrialSpectrograms_JNeurosci2023(rawDataFileIDs,neuralDataTypes);
% create Baselines data structure
baselineType = 'setDuration';
trialDuration_sec = 900;
targetMinutes = 60;
[RestingBaselines] = CalculateRestingBaselines_JNeurosci2023(animalID,targetMinutes,trialDuration_sec,RestData);
% Find spectrogram baselines for each day
[RestingBaselines] = CalculateSpectrogramBaselines_JNeurosci2023(animalID,neuralDataTypes,trialDuration_sec,RestingBaselines,baselineType);
% Normalize spectrogram by baseline
NormalizeSpectrograms_JNeurosci2023(neuralDataTypes,RestingBaselines);
% manually select files for custom baseline calculation
hemoType = 'reflectance';
[RestingBaselines] = CalculateManualRestingBaselinesTimeIndeces_JNeurosci2023(imagingType,hemoType);
% add delta HbT field to each processed data file
updatedBaselineType = 'manualSelection';
UpdateTotalHemoglobin_JNeurosci2023(procDataFileIDs,RestingBaselines,updatedBaselineType,imagingType,ledColor)
if strcmpi(imagingType,'GCaMP') == true
    CorrectGCaMPattenuation_JNeurosci2023(procDataFileIDs,RestingBaselines)
end
% re-create the RestData structure now that HbT is available
[RestData] = ExtractRestingData_JNeurosci2023(procDataFileIDs,updatedDataTypes,imagingType);
% create the EventData structure for CBV and neural data
[EventData] = ExtractEventTriggeredData_JNeurosci2023(procDataFileIDs,updatedDataTypes,imagingType);
...
[RestData] = NormBehavioralDataStruct_JNeurosci2023(RestData,RestingBaselines,updatedBaselineType);
save([animalID '_RestData.mat'],'RestData','-v7.3')
[EventData] = NormBehavioralDataStruct_JNeurosci2023(EventData,RestingBaselines,updatedBaselineType);
save([animalID '_EventData.mat'],'EventData','-v7.3')
% analyze the spectrogram baseline for each session.
% find spectrogram baselines for each day
[RestingBaselines] = CalculateSpectrogramBaselines_JNeurosci2023(animalID,neuralDataTypes,trialDuration_sec,RestingBaselines,updatedBaselineType);
% normalize spectrogram by baseline
NormalizeSpectrograms_JNeurosci2023(neuralDataTypes,RestingBaselines);
% create a structure with all spectrograms for convenient analysis further downstream
CreateAllSpecDataStruct_JNeurosci2023(animalID,neuralDataTypes)
...
if strcmpi(imagingType,'GCaMP') == true
    for bb = 1:size(procDataFileIDs,1)
        ...
        [figHandle] = GenerateSingleFigures_GCaMP_JNeurosci2023(procDataFileID,RestingBaselines,saveFigs,hemoType,'somatosensory');
        close(figHandle)
%         [figHandle] = GenerateSingleFigures_GCaMP_JNeurosci2023(procDataFileID,RestingBaselines,saveFigs,hemoType,'frontal');
%         close(figHandle)
    end
else
    for bb = 1:size(procDataFileIDs,1)
        procDataFileID = procDataFileIDs(bb,:);
        [figHandle] = GenerateSingleFigures_JNeurosci2023(procDataFileID,RestingBaselines,updatedBaselineType,saveFigs,imagingType,hemoType);
        close(figHandle)
    end
end
% isoflurane manual set
% SetIsofluraneHbT_JNeurosci2023()
% identify motion artifacts in neural data
% CheckNeuralMotionArtifacts_JNeurosci2023(procDataFileIDs,RestingBaselines,baselineType,imagingType,hemoType)
