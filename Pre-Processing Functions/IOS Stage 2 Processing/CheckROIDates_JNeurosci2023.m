function [ROIs] = CheckROIDates_JNeurosci2023(animalID,ROIs,ROInames,imagingType,lensMag)
...
[~,fileDates,~] = GetFileInfo_JNeurosci2023(windowCamDataFileIDs);
...
for b = 1:length(firstsFileOfDay)
    ...
    strDay = ConvertDate_JNeurosci2023(fileID);
        ...
        if ~isfield(ROIs,(ROIname))
            if any(strcmp(ROInames{1,c},{'LH','RH','frontalLH','frontalRH','Barrels'})) == true
                if strcmpi(imagingType,'GCaMP') == true
                    [ROIs] = PlaceGCaMP_ROIs_JNeurosci2023(animalID,fileID,ROIs,lensMag);
                else
                    [ROIs] = CalculateROICorrelationMatrix_JNeurosci2023(animalID,strDay,fileID,ROIs,imagingType,lensMag);
                end
            else
                [frames] = ReadDalsaBinary_JNeurosci2023(animalID,fileID);
                [ROIs] = CreateFreeHandROIs_JNeurosci2023(frames{3},ROIname,animalID,ROIs);
            end
