function [] = ExtractGCaMPData_Turner2022(ROIs,ROInames,rawDataFileIDs,procDataFileIDs)
...
for a = 1:size(rawDataFileIDs,1)
    ...
    [animalID,fileDate,fileID] = GetFileInfo_Turner2022(rawDataFileID);
    strDay = ConvertDate_Turner2022(fileDate);
    load(rawDataFileID)
    load(procDataFileID)
    [frames] = ReadDalsaBinary_Turner2022(animalID,[fileID '_WindowCam.bin']);
    ...
    for b = 1:length(ROInames)
        ...
        if any(strcmp(ROIshortName,{'LH','RH','frontalLH','frontalRH'})) == true
            ...
            close(maskFig)
            cbvMeanIntensity = BinToIntensity_Turner2022(mask,cbvFrames);
            gcampMeanIntensity = BinToIntensity_Turner2022(mask,gcampFrames);
            deoxyMeanIntensity = BinToIntensity_Turner2022(mask,deoxyFrames);
        ...
        elseif strcmp(ROIshortName,'Cement') == true
            ...
            close(maskFig)
            cbvCementMeanIntensity = BinToIntensity_Turner2022(mask,cbvFrames);
            gcampCementMeanIntensity = BinToIntensity_Turner2022(mask,gcampFrames);
            deoxyCementMeanIntensity = BinToIntensity_Turner2022(mask,deoxyFrames);
