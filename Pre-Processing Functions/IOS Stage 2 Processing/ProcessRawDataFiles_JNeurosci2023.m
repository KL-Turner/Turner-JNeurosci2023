function [] = ProcessRawDataFiles_JNeurosci2023(rawDataFiles)
...
for a = 1:size(rawDataFiles,1)
    ...
    [animalID,fileDate,fileID] = GetFileInfo_JNeurosci2023(rawDataFile);
    ...
    for c = 1:length(neuralDataTypes)
        ...
        % MUA [300 - 3000]
        [muaPower,~] = ProcessNeuro_JNeurosci2023(RawData,analogExpectedLength,'MUA',neuralDataType);
        ProcData.data.(neuralDataType).muaPower = muaPower;
        % gamma [30 - 100]
        [gammaBandPower,~] = ProcessNeuro_JNeurosci2023(RawData,analogExpectedLength,'Gam',neuralDataType);
        ProcData.data.(neuralDataType).gammaBandPower = gammaBandPower;
        % beta [13 - 30 Hz]
        [betaBandPower,~] = ProcessNeuro_JNeurosci2023(RawData,analogExpectedLength,'Beta',neuralDataType);
        ProcData.data.(neuralDataType).betaBandPower = betaBandPower;
        % alpha [10 - 13 Hz]
        [alphaBandPower,~] = ProcessNeuro_JNeurosci2023(RawData,analogExpectedLength,'Alpha',neuralDataType);
        ProcData.data.(neuralDataType).alphaBandPower = alphaBandPower;
        % theta [4 - 10 Hz]
        [thetaBandPower,~] = ProcessNeuro_JNeurosci2023(RawData,analogExpectedLength,'Theta',neuralDataType);
        ProcData.data.(neuralDataType).thetaBandPower = thetaBandPower;
        % delta [1 - 4 Hz]
        [deltaBandPower,~] = ProcessNeuro_JNeurosci2023(RawData,analogExpectedLength,'Delta',neuralDataType);
    ...
    [patchedWhisk] = PatchWhiskerAngle_JNeurosci2023(RawData.data.whiskerAngle,RawData.notes.whiskCamSamplingRate,RawData.notes.trialDuration_sec,RawData.notes.droppedWhiskCamFrameIndex);
    ...
    [ok] = CheckForThreshold_JNeurosci2023(['binarizedWhiskersLower_' strDay],animalID);
    if ok == 0
        [whiskersThresh1,whiskersThresh2] = CreateWhiskThreshold_JNeurosci2023(resampledWhisk,ProcData.notes.dsFs);
    ...
    binWhisk = BinarizeWhiskers_JNeurosci2023(resampledWhisk,ProcData.notes.dsFs,Thresholds.(['binarizedWhiskersLower_' strDay]),Thresholds.(['binarizedWhiskersUpper_' strDay]));
    [linkedBinarizedWhiskers] = LinkBinaryEvents_JNeurosci2023(gt(binWhisk,0),[round(ProcData.notes.dsFs/3),0]);
    ...
    [ok] = CheckForThreshold_JNeurosci2023(['binarizedForceSensor_' strDay],animalID);
    if ok == 0
        [forceSensorThreshold] = CreateForceSensorThreshold_JNeurosci2023(ProcData.data.forceSensor);
        Thresholds.(['binarizedForceSensor_' strDay]) = forceSensorThreshold;
        save([animalID '_Thresholds.mat'],'Thresholds');
    end
    ProcData.data.binForceSensor = BinarizeForceSensor_JNeurosci2023(ProcData.data.forceSensor,Thresholds.(['binarizedForceSensor_' strDay]));
