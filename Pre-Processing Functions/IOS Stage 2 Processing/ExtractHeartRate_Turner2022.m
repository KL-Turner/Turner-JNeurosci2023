function [] = ExtractHeartRate_Turner2022(procDataFiles,imagingType)
...
for a = 1:size(procDataFiles,1)
    ...
    if strcmpi(imagingType,'bilateral') == true
        ...
        [~,~,~,LH_HR] = FindHeartRate_Turner2022(ProcData.data.CBV.LH,ProcData.notes.CBVCamSamplingRate);
        [~,~,~,RH_HR] = FindHeartRate_Turner2022(ProcData.data.CBV.RH,ProcData.notes.CBVCamSamplingRate);
        % average the two signals from the left and right windows
        HR = (LH_HR + RH_HR)/2;
    elseif strcmpi(imagingType,'single') == true
        [~,~,~,HR] = FindHeartRate_Turner2022(ProcData.data.CBV.Barrels,ProcData.notes.CBVCamSamplingRate);
