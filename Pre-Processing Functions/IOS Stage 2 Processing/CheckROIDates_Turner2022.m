function [ROIs] = CheckROIDates_Turner2022(animalID,ROIs,ROInames,imagingType,lensMag)
...
[~,fileDates,~] = GetFileInfo_Turner2022(windowCamDataFileIDs);
...
for b = 1:length(firstsFileOfDay)
    ...
    strDay = ConvertDate_Turner2022(fileID);
        ...
        if ~isfield(ROIs,(ROIname))
            if any(strcmp(ROInames{1,c},{'LH','RH','frontalLH','frontalRH','Barrels'})) == true
                if strcmpi(imagingType,'GCaMP') == true
                    [ROIs] = PlaceGCaMP_ROIs_Turner2022(animalID,fileID,ROIs,lensMag);
                else
                    [ROIs] = CalculateROICorrelationMatrix_Turner2022(animalID,strDay,fileID,ROIs,imagingType,lensMag);
                end
            else
                [frames] = ReadDalsaBinary_Turner2022(animalID,fileID);
                [ROIs] = CreateFreeHandROIs_Turner2022(frames{3},ROIname,animalID,ROIs);
            end
