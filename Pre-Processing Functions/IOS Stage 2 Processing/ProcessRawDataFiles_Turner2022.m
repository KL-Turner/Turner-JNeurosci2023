function [] = ProcessRawDataFiles_Turner2022(rawDataFiles)
...
for a = 1:size(rawDataFiles,1)
    ...
    [animalID,fileDate,fileID] = GetFileInfo_Turner2022(rawDataFile);
    ...
    for c = 1:length(neuralDataTypes)
        ...
        % MUA [300 - 3000]
        [muaPower,~] = ProcessNeuro_Turner2022(RawData,analogExpectedLength,'MUA',neuralDataType);
        ProcData.data.(neuralDataType).muaPower = muaPower;
        % gamma [30 - 100]
        [gammaBandPower,~] = ProcessNeuro_Turner2022(RawData,analogExpectedLength,'Gam',neuralDataType);
        ProcData.data.(neuralDataType).gammaBandPower = gammaBandPower;
        % beta [13 - 30 Hz]
        [betaBandPower,~] = ProcessNeuro_Turner2022(RawData,analogExpectedLength,'Beta',neuralDataType);
        ProcData.data.(neuralDataType).betaBandPower = betaBandPower;
        % alpha [10 - 13 Hz]
        [alphaBandPower,~] = ProcessNeuro_Turner2022(RawData,analogExpectedLength,'Alpha',neuralDataType);
        ProcData.data.(neuralDataType).alphaBandPower = alphaBandPower;
        % theta [4 - 10 Hz]
        [thetaBandPower,~] = ProcessNeuro_Turner2022(RawData,analogExpectedLength,'Theta',neuralDataType);
        ProcData.data.(neuralDataType).thetaBandPower = thetaBandPower;
        % delta [1 - 4 Hz]
        [deltaBandPower,~] = ProcessNeuro_Turner2022(RawData,analogExpectedLength,'Delta',neuralDataType);
    ...
    [patchedWhisk] = PatchWhiskerAngle_Turner2022(RawData.data.whiskerAngle,RawData.notes.whiskCamSamplingRate,RawData.notes.trialDuration_sec,RawData.notes.droppedWhiskCamFrameIndex);
    ...
    [ok] = CheckForThreshold_Turner2022(['binarizedWhiskersLower_' strDay],animalID);
    if ok == 0
        [whiskersThresh1,whiskersThresh2] = CreateWhiskThreshold_Turner2022(resampledWhisk,ProcData.notes.dsFs);
    ...
    binWhisk = BinarizeWhiskers_Turner2022(resampledWhisk,ProcData.notes.dsFs,Thresholds.(['binarizedWhiskersLower_' strDay]),Thresholds.(['binarizedWhiskersUpper_' strDay]));
    [linkedBinarizedWhiskers] = LinkBinaryEvents_Turner2022(gt(binWhisk,0),[round(ProcData.notes.dsFs/3),0]);
    ...
    [ok] = CheckForThreshold_Turner2022(['binarizedForceSensor_' strDay],animalID);
    if ok == 0
        [forceSensorThreshold] = CreateForceSensorThreshold_Turner2022(ProcData.data.forceSensor);
        Thresholds.(['binarizedForceSensor_' strDay]) = forceSensorThreshold;
        save([animalID '_Thresholds.mat'],'Thresholds');
    end
    ProcData.data.binForceSensor = BinarizeForceSensor_Turner2022(ProcData.data.forceSensor,Thresholds.(['binarizedForceSensor_' strDay]));
