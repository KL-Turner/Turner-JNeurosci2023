function [] = PupilTrackerDemo_Turner2022(procDataFileIDs)
...
if isfield(PupilData,'firstFileOfDay') == false
    ...
    [~,fileDates,~] = GetFileInfo_Turner2022(procDataFileIDs);
    [uniqueDays,~,DayID] = GetUniqueDays_Turner2022(fileDates);
    for aa = 1:length(uniqueDays)
        strDay = ConvertDate_Turner2022(uniqueDays{aa,1});
        FileInd = DayID == aa;
        dayFilenames.(strDay) = procDataFileIDs(FileInd,:);
    end
    for bb = 1:length(uniqueDays)
        strDay = ConvertDate_Turner2022(uniqueDays{bb,1});
...
for bb = 1:length(firstFileOfDay)
    ...
    load(firstFile)
    [animalID,fileDate,fileID] = GetFileInfo_Turner2022(firstFile);
...
for cc = 1:size(procDataFileIDs,1)
    ...
    if isfield(ProcData.data.Pupil,'algorithmUpdate') == false
        ...
        [linkedBlinkIndex] = LinkBinaryEvents_Turner2022(gt(blinkNaNs,0),[samplingRate,0]); % link greater than 1 second
        ...
        [linkedDiffIndex] = LinkBinaryEvents_Turner2022(gt(diffIndex,0),[samplingRate*2,0]);
        % identify edges for interpolation
        edgeFoundB = false;
        xx = 1;
        startEdgeB = [];
