function [Results_PreWhitenedPowerSpectrum] = AnalyzePreWhitenedPowerSpectrum_JNeurosci2023(animalID,rootFolder,delim,Results_PreWhitenedPowerSpectrum)
...
for aa = 1:length(dataTypes)
    ...
    [nremData,~,~] = RemoveStimSleepData_JNeurosci2023(animalID,SleepData.(modelType).NREM.data.Pupil.(dataType).data,SleepData.(modelType).NREM.data.Pupil.fileIDs,SleepData.(modelType).NREM.data.Pupil.binTimes);
    ...
    if isempty(SleepData.(modelType).REM.data.Pupil) == false
        [remData,~,~] = RemoveStimSleepData_JNeurosci2023(animalID,SleepData.(modelType).REM.data.Pupil.(dataType).data,SleepData.(modelType).REM.data.Pupil.fileIDs,SleepData.(modelType).REM.data.Pupil.binTimes);
    else
        remData = [];
    end
