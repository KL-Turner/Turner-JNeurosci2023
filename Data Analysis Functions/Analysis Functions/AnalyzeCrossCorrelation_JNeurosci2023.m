function [Results_CrossCorrelation] = AnalyzeCrossCorrelation_JNeurosci2023(animalID,rootFolder,delim,Results_CrossCorrelation)
...
for aa = 1:length(dataTypes)
    ...
    for bb = 1:length(pupilDataTypes)
        ...
        [restLogical] = FilterEvents_JNeurosci2023(RestData.Pupil.(pupilDataType),RestCriteria);
        [puffLogical] = FilterEvents_JNeurosci2023(RestData.Pupil.(pupilDataType),RestPuffCriteria);
        ...
        [finalRestData,~,~,~] = RemoveInvalidData_JNeurosci2023(restData,restFileIDs,restDurations,restEventTimes,ManualDecisions);
        [finalPupilRestData,~,~,~] = RemoveInvalidData_JNeurosci2023(pupilRestData,restFileIDs,restDurations,restEventTimes,ManualDecisions);
        ...
        for cc = 1:size(procDataFileIDs,1)
            ...
            [~,~,alertDataFileID] = GetFileInfo_JNeurosci2023(procDataFileID);
            scoringLabels = [];
        ...
        for cc = 1:size(procDataFileIDs,1)
            procDataFileID = procDataFileIDs(cc,:);
            [~,~,asleepDataFileID] = GetFileInfo_JNeurosci2023(procDataFileID);
            scoringLabels = [];
            for dd = 1:length(ScoringResults.fileIDs)
        ...
        for cc = 1:size(procDataFileIDs,1)
            procDataFileID = procDataFileIDs(cc,:);
            [~,~,~] = GetFileInfo_JNeurosci2023(procDataFileID);
            load(procDataFileID,'-mat')
            % only run on files with good pupil measurement
            if strcmp(ProcData.data.Pupil.diameterCheck,'y') == true
                try
        ...
        if isempty(SleepData.(modelType).NREM.data.Pupil) == false
            ...
            [NREM_finalData,~,~] = RemoveStimSleepData_JNeurosci2023(animalID,SleepData.(modelType).NREM.data.Pupil.(dataType).data,SleepData.(modelType).NREM.data.Pupil.fileIDs,SleepData.(modelType).NREM.data.Pupil.binTimes);
            [NREM_finalPupilData,~,~] = RemoveStimSleepData_JNeurosci2023(animalID,SleepData.(modelType).NREM.data.Pupil.(pupilDataType).data,SleepData.(modelType).NREM.data.Pupil.fileIDs,SleepData.(modelType).NREM.data.Pupil.binTimes);
        ...
        if isempty(SleepData.(modelType).REM.data.Pupil) == false
            ...
            [REM_finalData,~,~] = RemoveStimSleepData_JNeurosci2023(animalID,SleepData.(modelType).REM.data.Pupil.(dataType).data,SleepData.(modelType).REM.data.Pupil.fileIDs,SleepData.(modelType).REM.data.Pupil.binTimes);
            [REM_finalPupilData,~,~] = RemoveStimSleepData_JNeurosci2023(animalID,SleepData.(modelType).REM.data.Pupil.(pupilDataType).data,SleepData.(modelType).REM.data.Pupil.fileIDs,SleepData.(modelType).REM.data.Pupil.binTimes);
