function [Results_Coherence] = AnalyzeCoherence_JNeurosci2023(animalID,rootFolder,delim,Results_Coherence)
...
for zzz = 1:length(hemDataTypes)
    ...
    for aa = 1:length(dataTypes)
        ...
        [HbT_finalRestData,~,~,~] = RemoveInvalidData_JNeurosci2023(HbT_restData,restFileIDs,restDurations,restEventTimes,ManualDecisions);
        [Pupil_FinalRestData,~,~,~] = RemoveInvalidData_JNeurosci2023(Pupil_restData,restFileIDs,restDurations,restEventTimes,ManualDecisions);
        ...
        for bb = 1:size(procDataFileIDs,1)
            ...
            [~,fileDate,alertDataFileID] = GetFileInfo_JNeurosci2023(procDataFileID);
        ...
        for bb = 1:size(procDataFileIDs,1)
            procDataFileID = procDataFileIDs(bb,:);
            [~,fileDate,asleepDataFileID] = GetFileInfo_JNeurosci2023(procDataFileID);
            strDay = ConvertDate_JNeurosci2023(fileDate);
            scoringLabels = [];
            for cc = 1:length(ScoringResults.fileIDs)
        ...
        if isempty(SleepData.(modelType).NREM.data.Pupil) == false
            [HbT_nremData,~,~] = RemoveStimSleepData_JNeurosci2023(animalID,SleepData.(modelType).NREM.data.Pupil.(hemDataType).data,SleepData.(modelType).NREM.data.Pupil.fileIDs,SleepData.(modelType).NREM.data.Pupil.binTimes);
            [Pupil_nremData,~,~] = RemoveStimSleepData_JNeurosci2023(animalID,SleepData.(modelType).NREM.data.Pupil.(dataType).data,SleepData.(modelType).NREM.data.Pupil.fileIDs,SleepData.(modelType).NREM.data.Pupil.binTimes);
        else
            HbT_nremData = [];
            Pupil_nremData = [];
        end
        ...
        if isempty(SleepData.(modelType).REM.data.Pupil) == false
            [HbT_remData,~,~] = RemoveStimSleepData_JNeurosci2023(animalID,SleepData.(modelType).REM.data.Pupil.(hemDataType).data,SleepData.(modelType).REM.data.Pupil.fileIDs,SleepData.(modelType).REM.data.Pupil.binTimes);
            [Pupil_remData,~,~] = RemoveStimSleepData_JNeurosci2023(animalID,SleepData.(modelType).REM.data.Pupil.(dataType).data,SleepData.(modelType).REM.data.Pupil.fileIDs,SleepData.(modelType).REM.data.Pupil.binTimes);
        else
            HbT_remData = [];
            Pupil_remData = [];
        end
