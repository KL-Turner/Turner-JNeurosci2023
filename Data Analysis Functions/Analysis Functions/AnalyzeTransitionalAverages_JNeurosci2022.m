function [Results_Transitions] = AnalyzeTransitionalAverages_JNeurosci2022(animalID,rootFolder,delim,Results_Transitions)
...
for h = 1:length(transitions)
    ...
    for i = 1:length(data.(transition).files)
        ...
        if startBin > 1 && startBin < (180 - 12)
            [animalID,fileDate,fileID] = GetFileInfo_JNeurosci2022(file);
