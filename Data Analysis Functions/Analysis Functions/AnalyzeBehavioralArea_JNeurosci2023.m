function [Results_BehavData] = AnalyzeBehavioralArea_JNeurosci2023(animalID,rootFolder,delim,Results_BehavData)
...
for aa = 1:length(dataTypes)
    ...
    [restLogical] = FilterEvents_JNeurosci2023(RestData.Pupil.(dataType),RestCriteria);
    [puffLogical] = FilterEvents_JNeurosci2023(RestData.Pupil.(dataType),RestPuffCriteria);
    ...
    [finalRestData,finalRestFileIDs,~,~] = RemoveInvalidData_JNeurosci2023(restData,restFileIDs,restDurations,restEventTimes,ManualDecisions);
    % filter
    ...
    [whiskLogical] = FilterEvents_JNeurosci2023(EventData.Pupil.(dataType).whisk,WhiskCriteria);
    [puffLogical] = FilterEvents_JNeurosci2023(EventData.Pupil.(dataType).whisk,WhiskPuffCriteria);
    ...
    % keep only the data that occurs within the manually-approved awake regions
    [finalWhiskData,finalWhiskFileIDs,~,~] = RemoveInvalidData_JNeurosci2023(whiskData,whiskFileIDs,whiskDurations,whiskEventTimes,ManualDecisions);
    ...
    % pull data from EventData.mat structure
    LH_stimFilter = FilterEvents_JNeurosci2023(EventData.Pupil.(dataType).stim,StimCriteriaA);
    RH_stimFilter = FilterEvents_JNeurosci2023(EventData.Pupil.(dataType).stim,StimCriteriaB);
    ...
    % keep only the data that occurs within the manually-approved awake regions
    [LH_finalStimData,LH_finalStimFileIDs,~,~] = RemoveInvalidData_JNeurosci2023(LH_stimData,LH_stimFileIDs,LH_stimDurations,LH_stimEventTimes,ManualDecisions);
    [RH_finalStimData,RH_finalStimFileIDs,~,~] = RemoveInvalidData_JNeurosci2023(RH_stimData,RH_stimFileIDs,RH_stimDurations,RH_stimEventTimes,ManualDecisions);
    ...
    if isempty(SleepData.(modelType).NREM.data.Pupil) == false
        [nremData,nremFileIDs,~] = RemoveStimSleepData_JNeurosci2023(animalID,SleepData.(modelType).NREM.data.Pupil.(dataType).data,SleepData.(modelType).NREM.data.Pupil.fileIDs,SleepData.(modelType).NREM.data.Pupil.binTimes);
    ...
    if isempty(SleepData.(modelType).REM.data.Pupil) == false
        [remData,remFileIDs,~] = RemoveStimSleepData_JNeurosci2023(animalID,SleepData.(modelType).REM.data.Pupil.(dataType).data,SleepData.(modelType).REM.data.Pupil.fileIDs,SleepData.(modelType).REM.data.Pupil.binTimes);
