function [Results_Evoked] = AnalyzeEvokedResponses_Turner2022(animalID,rootFolder,delim,Results_Evoked)
...
for aa = 1:length(dataTypes)
    ...
    for bb = 1:length(WhiskCriteriaNames)
        ...
        if strcmp(dataType,'LH_HbT') == true
            [whiskLogical] = FilterEvents_Turner2022(EventData.CBV_HbT.adjLH.whisk,WhiskCriteria);
            ...
            [finalWhiskData,~,~,~] = RemoveInvalidData_Turner2022(allWhiskData,allWhiskFileIDs,allWhiskDurations,allWhiskEventTimes,ManualDecisions);
        elseif strcmp(dataType,'RH_HbT') == true
            [whiskLogical] = FilterEvents_Turner2022(EventData.CBV_HbT.adjRH.whisk,WhiskCriteria);
            combWhiskLogical = logical(whiskLogical);
    ...
    for gg = 1:length(stimCriteriaNames)
        ...
        % pull data from EventData.mat structure
        if strcmp(dataType,'LH_HbT') == true
            allStimFilter = FilterEvents_Turner2022(EventData.CBV_HbT.adjLH.stim,StimCriteria);
            ...
            % keep only the data that occurs within the manually-approved awake regions
            [finalStimData,~,~,~] = RemoveInvalidData_Turner2022(allStimData,allStimFileIDs,allStimDurations,allStimEventTimes,ManualDecisions);
        elseif strcmp(dataType,'RH_HbT') == true
            allStimFilter = FilterEvents_Turner2022(EventData.CBV_HbT.adjRH.stim,StimCriteria);
            ...
            allStimFilter = FilterEvents_Turner2022(EventData.Pupil.(dataType).stim,StimCriteria);
