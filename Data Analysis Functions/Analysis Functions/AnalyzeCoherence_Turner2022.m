function [Results_Coherence] = AnalyzeCoherence_Turner2022(animalID,rootFolder,delim,Results_Coherence)
...
for zzz = 1:length(hemDataTypes)
    ...
    for aa = 1:length(dataTypes)
        ...
        [HbT_finalRestData,~,~,~] = RemoveInvalidData_Turner2022(HbT_restData,restFileIDs,restDurations,restEventTimes,ManualDecisions);
        [Pupil_FinalRestData,~,~,~] = RemoveInvalidData_Turner2022(Pupil_restData,restFileIDs,restDurations,restEventTimes,ManualDecisions);
        ...
        if isempty(SleepData.(modelType).NREM.data.Pupil) == false
            [HbT_nremData,~,~] = RemoveStimSleepData_Turner2022(animalID,SleepData.(modelType).NREM.data.Pupil.(hemDataType).data,SleepData.(modelType).NREM.data.Pupil.fileIDs,SleepData.(modelType).NREM.data.Pupil.binTimes);
            [Pupil_nremData,~,~] = RemoveStimSleepData_Turner2022(animalID,SleepData.(modelType).NREM.data.Pupil.(dataType).data,SleepData.(modelType).NREM.data.Pupil.fileIDs,SleepData.(modelType).NREM.data.Pupil.binTimes);
        else
            HbT_nremData = [];
            Pupil_nremData = [];
        end
        ...
        if isempty(SleepData.(modelType).REM.data.Pupil) == false
            [HbT_remData,~,~] = RemoveStimSleepData_Turner2022(animalID,SleepData.(modelType).REM.data.Pupil.(hemDataType).data,SleepData.(modelType).REM.data.Pupil.fileIDs,SleepData.(modelType).REM.data.Pupil.binTimes);
            [Pupil_remData,~,~] = RemoveStimSleepData_Turner2022(animalID,SleepData.(modelType).REM.data.Pupil.(dataType).data,SleepData.(modelType).REM.data.Pupil.fileIDs,SleepData.(modelType).REM.data.Pupil.binTimes);
        else
            HbT_remData = [];
            Pupil_remData = [];
        end
