function [] = Fig1_JNeurosci2023(rootFolder,saveFigs,delim)
...
% subplot for eye ROI
Fig1A = figure('Name','Figure Panel 1 - Turner et al. 2023','Units','Normalized','OuterPosition',[0,0,1,1]);
subplot(2,2,1)
...
if saveFigs == true
    ...
    savefig(Fig1A,[dirpath 'Fig1A_JNeurosci2023']);
    set(Fig1A,'PaperPositionMode','auto');
    print('-vector','-dpdf','-fillpage',[dirpath 'Fig1A_JNeurosci2023'])
    % text diary
...
Fig1B = figure('Name','Figure Panel 1 - Turner et al. 2023','Units','Normalized','OuterPosition',[0,0,1,1]);
...
if saveFigs == true
    ...
    savefig(Fig1B,[dirpath 'Fig1B_JNeurosci2023']);
    set(Fig1B,'PaperPositionMode','auto');
    print('-vector','-dpdf','-fillpage',[dirpath 'Fig1B_JNeurosci2023'])
end
%% example trial
Fig1C =  figure('Name','Figure Panel 1 - Turner et al. 2023','Units','Normalized','OuterPosition',[0,0,1,1]);
...
if saveFigs == true
    ...
    savefig(Fig1C,[dirpath 'Fig1C_JNeurosci2023']);
    % remove surface subplots because they take forever to render
    cla(ax6);
    set(ax6,'YLim',[1,99]);
    set(Fig1C,'PaperPositionMode','auto');
    print('-vector','-dpdf','-fillpage',[dirpath 'Fig1C_JNeurosci2023'])
    close(Fig1C)
    % subplot figure
    subplotImgs = figure('Name','Figure Panel 1 - Turner et al. 2023','Units','Normalized','OuterPosition',[0,0,1,1]);
    ...
    axis off
    xlim([0,900])
    print('-vector','-dtiffn',[dirpath 'Fig1_SpecImages_JNeurosci2023'])
    close(subplotImgs)
    % remake original figure
    figure('Name','Figure Panel 1 - Turner et al. 2023','Units','Normalized','OuterPosition',[0,0,1,1]);
    sgtitle('Example trial')
    % pupil
    ax12 = subplot(6,1,[1,2]);
    p1 = plot((1:length(Results_Example.filtPupilDiameter))/Results_Example.dsFs,Results_Example.filtPupilDiameter,'color',colors('black'));
